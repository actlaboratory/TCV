...
class updateDialog(BaseDialog):
	...
	def InstallControls(self):
		...
		if self.info["code"] == 200:
			...
			self.gauge.Disable()
			self.infoEdit, self.infoEditStatic = self.creator.inputbox(_("version %s アップデート情報") % (self.info["update_version"]), defaultValue = self.info["update_description"], x=600, style = wx.TE_MULTILINE | wx.TE_READONLY)
			...
			self.infoEdit, self.infoEditStatic = self.creator.inputbox(_("緊急のお知らせがあります。"), defaultValue = self.info["info_description"], style = wx.TE_MULTILINE | wx.TE_READONLY)
